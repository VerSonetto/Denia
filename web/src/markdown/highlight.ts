 ...
 * set is imported lazily and registered the first time such a language is
 * requested, so a session that never renders one of those languages pays
 * neither their grammar modules nor their synchronous init. The first render
 ...
 * resolve to the TypeScript grammar rather than a separate one — an accepted
 * trade to keep the boot set to one JS-family grammar.
 */
const LANGS = [langTs, langBash, langJson]

/**
 * The extension grammars, each behind a dynamic import so its module stays
 * out of the boot chunk until a fence of that language renders. Keyed by the
 * grammar id the aliases resolve to. The three boot grammars are absent —
 * already loaded.
 */
 ...
 * their first match; that compilation counts against Shiki's per-line budget
 * and can return a partial token stream under host contention. Eager
 ...
 * re-highlight. Uses the `useSyncExternalStore` subscribe signature; pair it
 * with {@link grammarLoadCount} as the snapshot.
 ...
 * renders its plain fallback. A lazy grammar not yet loaded returns
 * `undefined` for this call and loads in the background; subscribe with
 ...
 * swap render one identical span tree.
 */
 ...
 * still-growing last line. Non-append input and a change of resolved grammar
 * reset the cache and re-tokenize fully, so any input stays correct.
 */
export class StreamingHighlightSession {
  ...
  update(code: string, lang: string | undefined): readonly HighlightSpan[][] | undefined {
    ...
    if (lastNewline >= 0) {
      ...
      // the \r of a \r\n terminator, so a CRLF cut must not leak its \r into
      // the last completed line.
