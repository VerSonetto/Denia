/**
 * Direct mdast→React markdown renderer. One switch over parsed nodes so
 * streaming can cache frozen blocks as React elements.
 *
 * Untrusted-output policy: link and image destinations pass a protocol
 * allowlist, images additionally require absolute HTTP(S), raw HTML renders
 * as literal text (no HTML enters the DOM), and KaTeX runs without trusted
 ...
 * types) are dropped rather than kept as null placeholders.
...
/**
 * Interleave newline text nodes between block-level children. They are
 * invisible between elements but coalesce into adjacent literal raw-HTML text.
 * @param elements - Rendered block children with empty renders already dropped.
 * @param edges - Also emit the leading and trailing newline.
...
function renderCode(node: Md.Code, key: Key, context: MarkdownRenderContext): ReactNode {
  ...
  // The grammar id is recovered from the first word of the info string,
  // which truncates at the first non-word character.
  ...
  return (
    <CodeBlock
      key={key}
      // The replaced pipeline appended one synthetic newline that CodeBlock's
      // display trim removes; feeding the bare value would make that trim eat
      // a REAL trailing blank line inside the fence instead.
      code={`${node.value}\n`}
      lang={lang}
...
function renderTable(node: Md.Table, key: Key, context: MarkdownRenderContext): ReactNode {
  ...
  // table at natural width; narrower tables — and any table inside a
  // blockquote — fill the column and wrap instead.
  const wide = columns >= 4 && context.inBlockquote !== true
  return (
    // Wide tables rest with overflow-x hidden (the hover-revealed bar in the
    // design sheet), which drops Chromium's implicit scroller focusability —
    // the explicit tabindex keeps them keyboard-reachable.
    <div
      key={key}
      className={wide ? 'md-table-scroll md-table-wide' : 'md-table-scroll md-table-fill'}
...
function renderTableRow(
  ...
  // column, padding or truncating the row.
...
function renderImage(url: string, alt: string, key: Key): ReactNode {
  ...
  if (imageSrc === undefined) {
    return <span key={key} className="md-image-alt">{alt}</span>
  }
  return (
    <img
      key={key}
...
function renderLinkReference(
  ...
  if (definition === undefined) {
    // Incremental segments and hand-built trees may still present unresolved
    // references: revert to the bracketed source text.
    return <Fragment key={key}>{'['}{renderChildren(node.children, context)}{referenceSuffix(node)}</Fragment>
  }
  return renderAnchor(definition.url, renderChildren(node.children, context), key)
}
