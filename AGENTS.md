...
- **工具渲染消息右侧不加展开按钮**:展开/收起行为保留(点消息本体即可),但右侧别挂 chevron/箭头之类的控件,跟现有工具行一致。
...
- **每次提交前必须先获得用户明确批准**:agent 改完代码并通过编译/curl 验证后,先汇报改了什么、为什么这么改、构建结果,然后明确询问「是否提交本次改动」;**等用户回「提交」/「commit」/「OK」之类的明确许可**才能 `git commit`。不要把"做完了顺手提交"当默认行为。修改幅度小(单文件几行样式/小图标)可逐次批准,也可攒一批让用户一次性批准。
