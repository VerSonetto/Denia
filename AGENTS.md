...
- blank 会话(未发第一条消息)复用不新建;启动自动落最近工作区。
...
- 轮次关闭后,"最后一条工具调用之前"的过程内容折叠成一行"已工作 <时长> · N 次工具调用"概览(时长:<60秒直接秒数,≥60秒为分秒,≥1小时为时分秒),点击展开;最后一条工具调用与其后的回答保持可见,真实用户消息不折。运行中不设"过程"组:工具调用逐行平铺显示,过程透明可见。
